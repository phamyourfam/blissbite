 ...
 */
function findRouterFiles(dirPath: string, basePath: string = ''): Array<{filePath: string, routePath: string}> {
	const routerFiles: Array<{filePath: string, routePath: string}> = [];
 ...
 */
export default async function applyApiMiddleware(app: any): Promise<void> {
	const apiDirectory = __dirname;
	const entries = fs.readdirSync(apiDirectory);

	// Filter for directories that represent API versions (e.g., "v1", "v2")
	const versionDirectories = entries.filter((entry: string) => {
		const entryPath = path.join(apiDirectory, entry);
		return fs.statSync(entryPath).isDirectory() && entry.match(/^v\d+$/);
	});
	...
	for (const version of versionDirectories) {
		...
		const versionPath = path.join(apiDirectory, version);
		...
		for (const { filePath, routePath } of routerFiles) {
			try {
				...
				if (typeof createRouter === 'function') {
					const resourceRouter = createRouter(Router);

					if (!resourceRouter) {
						logger.error(
							`Router module ${filePath} returned nothing for route path "${routePath}" in version "${version}".`
						);
						continue;
					}
					...
					logger.info(
						`Mounted ${version}${normalizedRoutePath} router from ${filePath}`
					);
				} else {
					logger.error(
						`Router module ${filePath} does not export a router creation function`
					);
				}
			} catch (error) {
				if (error instanceof Error) {
					logger.error(
						`Failed to load API resource router: ${routePath} in ${version} ` +
						error.stack
					);
				} else {
					logger.error(
