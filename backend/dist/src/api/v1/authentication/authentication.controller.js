...
exports.completeSignup = exports.getCurrentUser = exports.logout = exports.login = exports.verifyEmailCode = exports.signupStart = void 0;
...
const keyvTempSignup = new keyv_1.default({ namespace: 'temp_signup' });
const keyvVerificationCodes = new keyv_1.default({ namespace: 'verification_codes' });
const keyvVerificationTokens = new keyv_1.default({ namespace: 'verification_tokens' });
// Constants from config
const SESSION_COOKIE_NAME = config_1.config.auth.sessionCookieName;
const TEMP_SIGNUP_DATA_TTL = config_1.config.auth.tempAccountTTL;
const VERIFICATION_CODE_TTL = config_1.config.auth.verificationCodeTTL;
const MAGIC_LINK_TOKEN_TTL = config_1.config.auth.magicLinkTTL;
// === Helper Functions ===
/**
...
const hashPassword = (password) => {
    const scrypt = new utils_1.Scrypt(password);
 ...
 */
const verifyPassword = (storedHash, providedPass) => {
    console.log(storedHash, providedPass);
    const [salt, hash] = storedHash.split(':');
    if (!salt || !hash) {
        throw new Error('Invalid stored password format.');
    }
    return utils_1.Scrypt.compare(providedPass, salt, hash);
};
/**
 ...
 */
const createSessionAndSetCookie = (ctx, accountId) => __awaiter(void 0, void 0, void 0, function* () {
 ...
 */
const validateSession = (ctx, sessionToken) => __awaiter(void 0, void 0, void 0, function* () {
    if (!sessionToken)
        return null;
 ...
 * @param ctx - Koa context (TypeORM enabled).
 */
const deleteSession = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    // Get the session token from the cookie
    const sessionToken = ctx.cookies.get(SESSION_COOKIE_NAME);
    if (sessionToken) {
        // Delete the session from the database
        const sessionRepository = ctx.getRepository('Session');
        yield sessionRepository.delete({ session_token: sessionToken });
    }
 ...
 */
const signupStart = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (resendCode && tempAccountId && email) {
        try {
            ...
            const tempSignupData = (yield keyvTempSignup.get(tempAccountId));
            if (!tempSignupData || tempSignupData.email !== email) {
                ctx.throw(404, 'Temporary account not found or expired.');
                return;
            }
            // Generate new verification code
            const verificationCode = (0, crypto_1.randomBytes)(3).toString('hex').toUpperCase(); // 6-char code
            yield keyvVerificationCodes.set(tempAccountId, verificationCode, VERIFICATION_CODE_TTL);
            // Send verification code via email
            try {
                yield (0, email_service_1.sendVerificationCode)(email, verificationCode, tempAccountId);
            }
            catch (emailError) {
                console.error('Failed to send verification email:', emailError);
                // Log but don't fail the request
            }
            console.log(`Resending verification code for ${email}. TempID: ${tempAccountId}, Code: ${verificationCode}`);
            ctx.status = 200;
            ctx.body = {
    ...
    const accountRepository = ctx.getRepository('Account');
    const existingAccount = yield accountRepository.findOneBy({ email });
    if (existingAccount) {
    ...
    try {
        ...
        const tempAccountId = `temp_${(0, crypto_1.randomBytes)(16).toString('hex')}`;
        ...
        yield verificationRepository.save(verification);
        ...
        yield keyvTempSignup.set(tempAccountId, signupData, TEMP_SIGNUP_DATA_TTL);
        ...
        console.log(`Signup started for ${email}. TempID: ${tempAccountId}, Code: ${verificationCode}`);
        ctx.status = 201;
        ctx.body = {
            tempAccountId,
            accountId: account.id
 ...
 */
const verifyEmailCode = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    const code = ctx.params.code;
    const { tempAccountId, accountId } = ctx.request.body;
    ...
    try {
        ...
        const storedCode = yield keyvVerificationCodes.get(tempAccountId);
        const tempSignupData = (yield keyvTempSignup.get(tempAccountId));
        ...
        const verificationToken = `verify_${(0, crypto_1.randomBytes)(32).toString('hex')}`;
        // Send magic link email for completing signup
        try {
            yield (0, email_service_1.sendMagicLink)(tempSignupData.email, verificationToken, tempAccountId);
        }
        catch (emailError) {
        ...
        yield keyvVerificationTokens.set(tempAccountId, verificationToken, MAGIC_LINK_TOKEN_TTL);
        // Clean up verification code as it's been used
        yield keyvVerificationCodes.delete(tempAccountId);
        ctx.status = 200;
        ctx.body = {
            success: true,
...
const login = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        console.log('Login attempt:', {
            ...
            hasStatus: (account === null || account === void 0 ? void 0 : account.status) ? true : false,
            status: (_a = account === null || account === void 0 ? void 0 : account.status) === null || _a === void 0 ? void 0 : _a.status
        });
        console.log(account);
        if (!account) {
        ...
        const verificationRepository = ctx.getRepository('AccountVerification');
        const emailVerification = yield verificationRepository.findOne({
            where: {
        ...
        yield createSessionAndSetCookie(ctx, account.id);
 ...
 */
const getCurrentUser = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    const sessionToken = ctx.cookies.get(SESSION_COOKIE_NAME);
    const accountId = yield validateSession(ctx, sessionToken || '');
    if (!accountId) {
 ...
 */
const completeSignup = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const storedToken = yield keyvVerificationTokens.get(tempAccountId);
        ...
        if (emailVerification) {
            ...
            yield verificationRepository.save(emailVerification);
        }
        ...
        yield accountStatusRepository.save(newStatus);
        // Associate the status with the account
        account.status = newStatus;
        yield accountRepository.save(account);
        // Ensure the account has the status relation loaded
        const updatedAccount = yield accountRepository.findOne({
            where: { id: account.id },
            relations: ['status']
        });
        // Cleanup temp data
        yield keyvTempSignup.delete(tempAccountId);
        yield keyvVerificationTokens.delete(tempAccountId);
        // Create session
        yield createSessionAndSetCookie(ctx, account.id);
        // Send welcome email
        try {
            yield (0, email_service_1.sendWelcomeEmail)(account.email, account.forename);
        }
        catch (emailError) {
